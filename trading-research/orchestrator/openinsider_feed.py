"""
OpenInsider live feed — primary insider-cluster discovery for v3.0.

Replaces the per-ticker SEC EDGAR scan (v1–v2.1) which required ~25–30 hours of cold-
cache scanning to cover the full $200M–$3B universe. OpenInsider aggregates SEC Form 4
filings into a queryable feed: a single HTTP scrape returns all qualifying open-market
insider purchases across the entire US market over a date range.

v3.0 architecture (this module + run_pipeline.py):
  ...
  3. Classify each cluster member as routine vs opportunistic (Cohen-Malloy-Pomorski 2012)
     using the full 3-year scrape — no separate EDGAR per-ticker calls required
  4. Hand off detected clusters to run_pipeline for yfinance enrichment + reporting

Per Cohen-Malloy-Pomorski (2012, JF): an insider is "routine" if they bought in the same
calendar month for 3 consecutive prior years. The routine classification uses the same
OpenInsider-scraped history that drives cluster detection — single data source.

Limitation: OpenInsider filters at the source to purchases ≥ $100K. Routine traders whose
habitual buys are smaller than $100K will not show up in the scraped history and will be
mis-classified as opportunistic. This biases conservatively (the opportunistic soft gate
in run_pipeline only blocks pure-routine clusters; mis-classifying a routine trader as
opportunistic means the cluster passes through, which is the safer error direction).

Scrape URL parameters mirror backtest/openinsider_pull.py for consistency between live
and backtest data paths.
"""
...
from orchestrator.insider_scanner import (
    CLUSTER_WINDOW_DAYS,
    InsiderCluster,
    InsiderTransaction,
...
def _is_entity_name(name: str) -> bool:
    n = name.lower().strip()
    return any(
        n.endswith(" " + s) or n.endswith(" " + s + ".") or n == s
...
def _fetch_screener_page(start: date, end: date, page: int = 1) -> str:
    ...
    Mirrors backtest/openinsider_pull.py's URL params so live and backtest data
    paths are consistent. nil=1 returns all purchase activity regardless of how
    many other insiders are trading on the same ticker.
    """
...
def _parse_table(html: str) -> list[InsiderTransaction]:
...
def _save_chunk_cache(path: Path, rows: list[tuple[str, InsiderTransaction]]) -> None:
    """Persist parsed transactions for one quarter chunk. CSV form keeps it inspectable."""
...
def detect_clusters(
    ...
    unique names. Emit at most ONE cluster per ticker (the best one), which fixes the
    rolling-window-duplicate bug in the backtest's detect_clusters().
    """
    today = date.today()
