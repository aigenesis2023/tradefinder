...
1. **Universe construction** — survivorship-bias-free, point-in-time index constituents, delisted stocks included with delisting returns. **Mandatory:** must use CRSP delisting data (or equivalent survivorship-bias-free database with full delisting return history). Must source point-in-time index constituent lists (not current constituents projected backward). Must include all stocks that existed at each point in time, including those that later delisted for any reason (bankruptcy, acquisition, exchange delisting, liquidation). The Data Breaker must specifically verify these requirements are met before the pipeline is locked.
